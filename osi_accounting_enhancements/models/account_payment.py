...
class AccountPayment(models.Model):
    ...
    def _get_shared_move_line_vals(self, debit, credit, amount_currency,
        ...
        rec = super(AccountPayment, self)._get_shared_move_line_vals(
            debit=debit, credit=credit, amount_currency=amount_currency,
            move_id=move_id, invoice_id=invoice_id)
        if not invoice_id and self._context.get(
        ...
        if invoice_id and len(invoice_id) == 1 and invoice_id.type in (
        ...
        rec.update({'invoice_id': id,
                    'supplier_invoice_number': sin,
                    'purchase_ref': pref})
        return rec